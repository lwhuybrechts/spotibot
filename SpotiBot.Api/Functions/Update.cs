...
namespace SpotiBot
{
    public class Update
    {
        ...
        public Update(
            ...
            IOptions<Api.Library.Options.SentryOptions> sentryOptions)
        {
        ...
        public async Task<IStatusCodeActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequest httpRequest)
        {
            ...
            using (new SentryExceptionHandler(_sentryOptions.Dsn, requestBody))
            {
                try
                {
                    var update = JsonSerializer.Deserialize<Telegram.Bot.Types.Update>(requestBody, Telegram.Bot.JsonBotAPI.Options);
